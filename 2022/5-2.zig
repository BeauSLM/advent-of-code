...
pub fn main() !void {
    ...
    while (initer.next()) |line| {
        ...
        const move = try std.fmt.parseInt(u8, numiter.next().?, 10);
        const from = try std.fmt.parseInt(u8, numiter.next().?, 10) - 1;
        const to = try std.fmt.parseInt(u8, numiter.next().?, 10) - 1;

        var src = &stacks[from];
        var dest = &stacks[to];
        const ix = src.items.len - move;
        var movedcrates = src.items[ix..src.items.len];
        // print("{s}\n", .{movedcrates});
        try dest.appendSlice(movedcrates);
        for (movedcrates) |_| {
            _ = src.pop();
        }
        // for (stacks) |stack| {
